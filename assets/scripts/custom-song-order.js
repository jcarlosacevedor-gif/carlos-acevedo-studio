(() => {
  const modal = document.querySelector("#custom-song-modal");
  const open = document.querySelector("[data-custom-song-open]");
  ...
  const text=(name)=>Boolean(f(name)?.value.trim());
  function fail(key,input){error.textContent=t(key);input?.focus();return false;}
  function validate(){if(step===1){if(!checked("purpose"))return fail("errorPurpose",first("purpose"));if(checked("purpose")==="other"&&!text("purposeOther"))return fail("errorPurposeOther",f("purposeOther"));if(!text("subject"))return fail("errorSubject",f("subject"));if(!text("story"))return fail("errorStory",f("story"));if(!checked("language"))return fail("errorLanguage",first("language"));if(checked("language")==="other"&&!text("languageOther"))return fail("errorLanguageOther",f("languageOther"));if(!f("genre").value)return fail("errorGenre",f("genre"));if(f("genre").value==="other"&&!text("genreOther"))return fail("errorGenreOther",f("genreOther"));if(!checks("mood").length)return fail("errorMood",first("mood"));if(checks("mood").includes("other")&&!text("moodOther"))return fail("errorMoodOther",f("moodOther"));}if(step===2){if(!checked("lyricsStatus"))return fail("errorLyrics",first("lyricsStatus"));if(checked("lyricsStatus")==="complete"&&!f("lyricsPermission").checked)return fail("errorPermission",f("lyricsPermission"));if(!checked("creativeFreedom"))return fail("errorFreedom",first("creativeFreedom"));}if(step===3){if(!checks("instrument").length)return fail("errorInstrument",first("instrument"));if(!checked("vocal"))return fail("errorVocal",first("vocal"));}if(step===5&&["contentGuidelines","revisionAcknowledgement","licenseAcknowledgement"].some(n=>!f(n).checked))return fail("errorAck",f("contentGuidelines"));error.textContent="";return true;}
  function row(label,value){if(!value)return;const d=document.createElement("div"),a=document.createElement("strong"),b=document.createElement("span");a.textContent=label;b.textContent=value;d.append(a,b);summary.append(d);}
  function renderSummary(){summary.replaceChildren();const other=(n,v)=>v==="other"?f(`${n}Other`).value.trim():labelFor(v,maps[n]);const genre=f("genre").value==="other"?f("genreOther").value.trim():f("genre").selectedOptions[0]?.textContent.trim(), solo=checked("solo"), extra=solo==="none"?0:25;row(t("summaryPurpose"),other("purpose",checked("purpose")));row(t("summarySubject"),f("subject").value.trim());row(t("summaryStory"),f("story").value.trim());row(t("summaryLanguage"),other("language",checked("language")));row(t("summaryGenre"),genre);row(t("summaryMood"),checks("mood").map(v=>other("mood",v)).join(", "));row(t("summaryLyrics"),labelFor(checked("lyricsStatus"),maps.lyricsStatus));row(t("summaryLyricsDetails"),f("lyricsDetails").value.trim());row(t("summaryAvoid"),f("avoid").value.trim());row(t("summaryFreedom"),labelFor(checked("creativeFreedom"),maps.creativeFreedom));row(t("summaryInstruments"),checks("instrument").map(v=>labelFor(v,maps.instrument)).join(", "));row(t("summaryVocal"),labelFor(checked("vocal"),maps.vocal));row(t("summarySolo"),labelFor(solo,maps.solo));row(t("summaryReferences"),f("references").value.trim());row(t("summaryAdditional"),f("additionalNotes").value.trim());soloLine.hidden=extra===0;soloPrice.hidden=extra===0;soloLine.textContent=extra?labelFor(solo,maps.solo).replace(" — +$25",""):"";total.textContent=`$${199+extra}`;}
  function show(n){step=n;steps.forEach(s=>s.hidden=Number(s.dataset.customSongStep)!==step);current.textContent=step;progress.style.width=`${step*20}%`;back.hidden=step===1;next.textContent=step===5?t("reviewComplete"):t("continue");error.textContent="";if(step===5)renderSummary();panel.scrollTop=0;panel.focus();}
